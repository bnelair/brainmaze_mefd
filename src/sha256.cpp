/**
 * @file sha256.cpp
 * @brief SHA-256 implementation for MEF 3.0
 * 
 * Based on FIPS 180-2 SHA-256 implementation by Olivier Gay.
 * Modified for compatibility with MEF Library.
 */

#include "brainmaze_mefd/sha256.hpp"
#include <cstring>

namespace brainmaze_mefd {

// SHA-256 Initial Hash Values
const std::array<ui4, 8> SHA256::s_h0 = {{
    0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
    0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
}};

// SHA-256 Round Constants
const std::array<ui4, 64> SHA256::s_k = {{
    0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
    0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
    0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
    0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
    0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
    0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
    0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
    0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
    0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
    0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
    0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
    0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
    0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
    0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
    0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
    0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
}};

// Helper macros
#define SHFR(x, n)   ((x) >> (n))
#define ROTR(x, n)   (((x) >> (n)) | ((x) << (32 - (n))))
#define CH(x, y, z)  (((x) & (y)) ^ (~(x) & (z)))
#define MAJ(x, y, z) (((x) & (y)) ^ ((x) & (z)) ^ ((y) & (z)))

#define SHA256_F1(x) (ROTR(x, 2) ^ ROTR(x, 13) ^ ROTR(x, 22))
#define SHA256_F2(x) (ROTR(x, 6) ^ ROTR(x, 11) ^ ROTR(x, 25))
#define SHA256_F3(x) (ROTR(x, 7) ^ ROTR(x, 18) ^ SHFR(x, 3))
#define SHA256_F4(x) (ROTR(x, 17) ^ ROTR(x, 19) ^ SHFR(x, 10))

#define PACK32(str, x) { \
    *(x) = ((ui4)*((str) + 3)) | \
           ((ui4)*((str) + 2) << 8) | \
           ((ui4)*((str) + 1) << 16) | \
           ((ui4)*((str) + 0) << 24); \
}

#define UNPACK32(x, str) { \
    *((str) + 3) = (ui1)(x); \
    *((str) + 2) = (ui1)((x) >> 8); \
    *((str) + 1) = (ui1)((x) >> 16); \
    *((str) + 0) = (ui1)((x) >> 24); \
}

SHA256::Context::Context() : m_total_len(0), m_len(0) {
    m_h = s_h0;
}

void SHA256::Context::transform(const ui1* message, ui4 block_nb) {
    ui4 w[64];
    ui4 wv[8];
    ui4 t1, t2;
    const ui1* sub_block;
    
    for (ui4 i = 0; i < block_nb; ++i) {
        sub_block = message + (i << 6);
        
        // Prepare message schedule
        for (int j = 0; j < 16; ++j) {
            PACK32(&sub_block[j << 2], &w[j]);
        }
        
        for (int j = 16; j < 64; ++j) {
            w[j] = SHA256_F4(w[j - 2]) + w[j - 7] + SHA256_F3(w[j - 15]) + w[j - 16];
        }
        
        // Initialize working variables
        for (int j = 0; j < 8; ++j) {
            wv[j] = m_h[j];
        }
        
        // Main loop
        for (int j = 0; j < 64; ++j) {
            t1 = wv[7] + SHA256_F2(wv[4]) + CH(wv[4], wv[5], wv[6]) + s_k[j] + w[j];
            t2 = SHA256_F1(wv[0]) + MAJ(wv[0], wv[1], wv[2]);
            
            wv[7] = wv[6];
            wv[6] = wv[5];
            wv[5] = wv[4];
            wv[4] = wv[3] + t1;
            wv[3] = wv[2];
            wv[2] = wv[1];
            wv[1] = wv[0];
            wv[0] = t1 + t2;
        }
        
        // Add to hash value
        for (int j = 0; j < 8; ++j) {
            m_h[j] += wv[j];
        }
    }
}

void SHA256::Context::update(const ui1* data, ui4 length) {
    ui4 block_nb;
    ui4 new_len, rem_len, tmp_len;
    const ui1* shifted_message;
    
    tmp_len = BLOCK_SIZE - m_len;
    rem_len = length < tmp_len ? length : tmp_len;
    
    std::memcpy(&m_block[m_len], data, rem_len);
    
    if (m_len + length < BLOCK_SIZE) {
        m_len += length;
        return;
    }
    
    new_len = length - rem_len;
    block_nb = new_len / BLOCK_SIZE;
    
    shifted_message = data + rem_len;
    
    transform(m_block.data(), 1);
    transform(shifted_message, block_nb);
    
    rem_len = new_len % BLOCK_SIZE;
    
    std::memcpy(m_block.data(), &shifted_message[block_nb << 6], rem_len);
    
    m_len = rem_len;
    m_total_len += (block_nb + 1) << 6;
}

SHA256::Digest SHA256::Context::finalize() {
    Digest digest;
    ui4 block_nb;
    ui4 pm_len;
    ui4 len_b;
    
    block_nb = (1 + ((BLOCK_SIZE - 9) < (m_len % BLOCK_SIZE)));
    
    len_b = (m_total_len + m_len) << 3;
    pm_len = block_nb << 6;
    
    std::memset(&m_block[m_len], 0, pm_len - m_len);
    m_block[m_len] = 0x80;
    
    UNPACK32(len_b, &m_block[pm_len - 4]);
    
    transform(m_block.data(), block_nb);
    
    for (int i = 0; i < 8; ++i) {
        UNPACK32(m_h[i], &digest[i << 2]);
    }
    
    return digest;
}

SHA256::Digest SHA256::hash(const ui1* data, ui4 length) {
    Context ctx;
    ctx.update(data, length);
    return ctx.finalize();
}

SHA256::Digest SHA256::hash(const std::string& str) {
    return hash(reinterpret_cast<const ui1*>(str.data()), static_cast<ui4>(str.length()));
}

} // namespace brainmaze_mefd
