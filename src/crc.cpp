...
// Pre-computed Koopman32 CRC table
std::array<ui4, CRC32::TABLE_ENTRIES> CRC32::s_table = {{
...
const std::array<ui4, CRC32::TABLE_ENTRIES>& CRC32::get_table() {
    return s_table;
}

std::array<ui4, CRC32::TABLE_ENTRIES> CRC32::initialize_table() {
    // The table is pre-computed above, but this function shows how it would be generated
    std::array<ui4, TABLE_ENTRIES> table{};
    for (size_t i = 0; i < TABLE_ENTRIES; ++i) {
        ui4 crc = static_cast<ui4>(i);
        for (int j = 0; j < 8; ++j) {
            if (crc & 1) {
                crc = (crc >> 1) ^ KOOPMAN32;
            } else {
